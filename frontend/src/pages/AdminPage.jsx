...
import { FiGrid, FiPackage, FiShoppingBag, FiPlus, FiEdit, FiTrash2, FiX, FiCheck } from "react-icons/fi";
...
const AdminPage = () => {
  ...
  const [productForm, setProductForm] = useState({
    ...
    sizes: "", colors: "", countInStock: "", category: "general", tag: "",
  });
  ...
  const uploadFileHandler = async (e) => {
    ...
    try {
      ...
      setProductForm((prev) => ({ ...prev, images: prev.images ? `${prev.images}, ${data.imageUrl}` : data.imageUrl }));
      setUploading(false);
    } catch (err) {
  ...
  const handleProductSubmit = async (e) => {
    ...
    try {
      const productData = {
        ...
        category: productForm.category, tag: productForm.tag,
      };
      if (editingProduct) {
  ...
  const handleEditProduct = (product) => {
    ...
    setProductForm({
      ...
      images: product.images.join(", "), sizes: product.sizes.join(", "),
      colors: product.colors.join(", "), countInStock: product.countInStock,
      category: product.category || "general", tag: product.tag || "",
    });
  ...
  const handleDeleteProduct = async (id) => {
    ...
    try { await API.delete(`/api/products/${id}`, config); fetchData(); } catch (err) { console.log(err); }
  };

  const handleMarkDelivered = async (orderId) => {
    try { await API.put(`/api/orders/${orderId}/deliver`, {}, config); fetchData(); } catch (err) { console.log(err); }
  };
  ...
  const inputStyle = { width: "100%", padding: "10px 12px", border: "1px solid #ddd", fontSize: "13px", outline: "none", boxSizing: "border-box" };
  const labelStyle = { display: "block", fontSize: "11px", fontWeight: "700", letterSpacing: "1px", color: "#444", marginBottom: "6px" };

  if (loading) return <div style={{ textAlign: "center", padding: "100px", color: "#999" }}>Loading dashboard...</div>;

  return (
    <div className="admin-layout" style={{ display: "flex", minHeight: "100vh", backgroundColor: "#f5f5f5" }}>

      {/* Sidebar */}
      <div className="admin-sidebar" style={{ width: "240px", backgroundColor: "#1a1a1a", color: "#fff", padding: "30px 0", flexShrink: 0, position: "sticky", top: 0, height: "100vh" }}>
        <div style={{ padding: "0 25px 30px", borderBottom: "1px solid #333", marginBottom: "20px" }}>
          <p style={{ fontSize: "11px", letterSpacing: "2px", color: "#c8a96e", marginBottom: "5px" }}>ADMIN PANEL</p>
        ...
        {[
          ...
          <button key={tab.id} onClick={() => setActiveTab(tab.id)} style={{
            width: "100%", padding: "14px 25px", display: "flex", alignItems: "center", gap: "12px",
            backgroundColor: activeTab === tab.id ? "#c8a96e" : "transparent",
            color: activeTab === tab.id ? "#fff" : "#999",
            border: "none", cursor: "pointer", fontSize: "13px", fontWeight: "600", letterSpacing: "1px", textAlign: "left", transition: "all 0.2s",
          }}>
            {tab.icon}{tab.label}
          </button>
        ))}
      ...
      <div className="admin-content" style={{ flex: 1, padding: "40px", overflowY: "auto" }}>
        ...
        {activeTab === "dashboard" && (
          <div>
            <h1 style={{ fontSize: "22px", fontWeight: "800", letterSpacing: "2px", color: "#222", marginBottom: "30px" }}>DASHBOARD OVERVIEW</h1>
            <div className="admin-stats-grid" style={{ marginBottom: "40px" }}>
              {[
                ...
                <div key={i} style={{ backgroundColor: "#fff", padding: "25px", border: "1px solid #f0f0f0", borderLeft: i === 0 ? "3px solid #c8a96e" : "1px solid #f0f0f0", boxShadow: "0 2px 10px rgba(0,0,0,0.02)" }}>
                  <p style={{ fontSize: "10px", fontWeight: "700", letterSpacing: "1.5px", color: "#999", marginBottom: "10px" }}>{stat.label}</p>
                  <p style={{ fontSize: "28px", fontWeight: "800", color: "#222", lineHeight: "1" }}>{stat.value}</p>
                </div>
              ))}
            </div>
            <div style={{ backgroundColor: "#fff", padding: "25px", boxShadow: "0 2px 10px rgba(0,0,0,0.05)", overflowX: "auto" }}>
              <h2 style={{ fontSize: "14px", fontWeight: "800", letterSpacing: "2px", color: "#222", marginBottom: "20px" }}>RECENT ORDERS</h2>
              <table style={{ width: "100%", borderCollapse: "collapse", minWidth: "500px" }}>
                <thead>
                  <tr style={{ borderBottom: "2px solid #eee" }}>
                    {["Order ID", "Customer", "Total", "Paid", "Delivered"].map((h) => (
                      <th key={h} style={{ padding: "10px", fontSize: "11px", fontWeight: "700", letterSpacing: "1px", color: "#999", textAlign: "left" }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
        ...
        {activeTab === "products" && (
          <div>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "30px" }}>
              <h1 style={{ fontSize: "22px", fontWeight: "800", letterSpacing: "2px", color: "#222" }}>PRODUCTS ({products.length})</h1>
              <button
                onClick={() => { setEditingProduct(null); setProductForm({ name: "", description: "", price: "", images: "", sizes: "", colors: "", countInStock: "", category: "general", tag: "" }); setShowProductForm(true); }}
            ...
            {showProductForm && (
              <div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, backgroundColor: "rgba(0,0,0,0.5)", zIndex: 1000, display: "flex", alignItems: "center", justifyContent: "center", padding: "20px" }}>
                <div style={{ backgroundColor: "#fff", padding: "35px", width: "100%", maxWidth: "580px", maxHeight: "90vh", overflowY: "auto" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "25px" }}>
                    <h2 style={{ fontSize: "16px", fontWeight: "800", letterSpacing: "2px", color: "#222" }}>{editingProduct ? "EDIT PRODUCT" : "ADD NEW PRODUCT"}</h2>
                    <button onClick={() => setShowProductForm(false)} style={{ background: "none", border: "none", cursor: "pointer", color: "#999" }}><FiX size={22} /></button>
                  </div>
                  <form onSubmit={handleProductSubmit}>
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "15px" }}>
                      ...
                      <div style={{ gridColumn: "1 / -1" }}>
                        <label style={labelStyle}>IMAGE URLS (comma separated) OR UPLOAD</label>
                        <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
                          <input name="images" value={productForm.images} onChange={handleProductFormChange} required placeholder="https://image1.jpg" style={inputStyle} />
            ...
            <div style={{ backgroundColor: "#fff", boxShadow: "0 2px 10px rgba(0,0,0,0.05)", overflowX: "auto" }}>
        ...
        {activeTab === "orders" && (
          <div>
            <h1 style={{ fontSize: "22px", fontWeight: "800", letterSpacing: "2px", color: "#222", marginBottom: "30px" }}>ALL ORDERS ({orders.length})</h1>
            <div style={{ backgroundColor: "#fff", boxShadow: "0 2px 10px rgba(0,0,0,0.05)", overflowX: "auto" }}>
        ...
        {activeTab === "myorders" && (
          <div>
            <h1 style={{ fontSize: "22px", fontWeight: "800", letterSpacing: "2px", color: "#222", marginBottom: "30px" }}>MY ORDERS</h1>
            {orders.filter((o) => o.user?._id === user?._id || o.user === user?._id).length === 0 ? (
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "15px" }}>
                {orders.filter((o) => o.user?._id === user?._id || o.user === user?._id).map((order) => (
                  <div key={order._id} className="responsive-flex" style={{ backgroundColor: "#fff", padding: "20px 25px", boxShadow: "0 2px 10px rgba(0,0,0,0.05)", display: "flex", justifyContent: "space-between", alignItems: "center", gap: "15px" }}>
                    <div>
